...
TcpGymEnv::GetTcpCongStateName(const TcpSocketState::TcpCongState_t state)
{
  ...
  NS_LOG_UNCOND("MyGetExtraInfo: " << stateName);
...
TcpGymEnv::GetTcpCAEventName(const TcpSocketState::TcpCAEvent_t event)
{
  ...
  NS_LOG_UNCOND("MyGetExtraInfo: " << eventName);
...
TcpGymEnv::GetActionSpace()
{
  ...
  //uint32_t parameterNum = 1; //only new_cWnd
  float low = 0.0;
  float high = 65535;
...
TcpTimeStepGymEnv::GetObservation()
{
  ...
  //NS_LOG_UNCOND("Throughput segmentsAckedsum: "<<segmentsAckedSum);
  NS_LOG_UNCOND("Throughput time: "<<m_timeStep.GetSeconds());
  float throughput = (segmentsAckedSum * m_tcb->m_segmentSize) / m_timeStep.GetSeconds();
  box->AddValue(throughput);

  // Print data
  NS_LOG_UNCOND ("Timebased MyGetObservation: " << box);
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_UNCOND(Simulator::Now() << " Node: " << m_nodeId << " GetSsThresh, BytesInFlight: " << bytesInFlight);
  m_tcb = tcb;
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_UNCOND(Simulator::Now() << " Node: " << m_nodeId << " PktsAcked, SegmentsAcked: " << segmentsAcked << " Rtt: " << rtt);
  m_tcb = tcb;
  m_rttSum += rtt;
  m_rttSampleNum++;
}
